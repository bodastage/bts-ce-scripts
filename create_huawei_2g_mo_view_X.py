...
for v in view_list:
    ...
    view = Table(v, metadata, autoload=True, autoload_with=engine, schema='huawei_cm_2g')
...
{0} = ReplaceableObject(
    ...
    print("    {}.\"{}\" t1".format('huawei_nbi_gsm', v))
    print("    ")

    mo_position = 1
    for mo in mo_list:
        mo_position += 1
        table = Table(mo, metadata, autoload=True, autoload_with=engine, schema='huawei_gexport_gsm')
        ...
        print("FROM")
        print("huawei_gexport_gsm.\"{}\" t{}".format(mo, mo_position))
        print("""INNER JOIN huawei_gexport_gsm."SYS_{1}" t{0}1 ON t{0}1."FILENAME" = t{0}."FILENAME\""""
            .format(mo_position, ne_type_for_gexport))
        

    for mo in mml_mo_list:
        mo_position += 1
        table = Table(mo, metadata, autoload=True, autoload_with=engine, schema='huawei_mml_gsm')
        columns1 = map(lambda x: x.name, table.columns)
        # logger.info(columns1)
        ...
        print("FROM")
        print("huawei_mml_gsm.\"{}\" t{}".format(mo, mo_position))
        print("""INNER JOIN huawei_mml_gsm."SYS" t{0}1 ON t{0}1."FileName" = t{0}."FileName"
        """.format(mo_position))
