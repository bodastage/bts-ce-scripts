# Generate load query for Huawei GExport WCDMA
#python create_huawei_gexport_wcdma_load_insert_queries.py
#
# Use huawei_cm_3g load table as the reference for MOs that need to be inserted
...
for mo in mo_list:
    ...
    AND ( table_name  = '{0}' OR table_name = 'U' || '{0}' )
    LIMIT 1
    """.format(mo)
    ...
    insert_query = ""
    insert_query += " INSERT INTO huawei_cm_3g.\"{}\"  ".format(mo)
    ...
    insert_query += "    FROM"
    insert_query += "    huawei_gexport_wcdma.\"{}\" t ".format(mo)
    insert_query += "    CROSS JOIN cm_loads t2  WHERE t2.is_current_load = true ".format(mo3)

    print("{{'file_format': 'huawei_gexport_wcdma', 'mo': '{}', 'insert_query': '{}'}},".format(mo, insert_query))
    # sys.exit(0)
